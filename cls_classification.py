...
from bert_model import Bert_Classification
import os
...
def test(model, dev_loader, use_cuda,ids):
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(dev_loader):
            ...
            predicts=torch.argmax(preds,dim=1)
            predict_labels+=predicts.cpu().numpy().tolist()
    print('test nums',len(predict_labels))
...
def main():
    ...
    model=Bert_Classification(config,tagset_size)
    if args.use_cuda:
        model = model.cuda()
    if submit:
        ...
        print('submimt loader numbers',len(submit_ids))
        ...
        #pooled_output_param_optimizer=list(model.pooler.named_parameters())
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': [p for n, p in bert_param_optimizer if any(nd in n for nd in no_decay)], 'weight_decay': 0.0,
             'lr': lr},
           #{'params': [p for n, p in pooled_output_param_optimizer if not any(nd in n for nd in no_decay)],
            # 'weight_decay': 0.01, 'lr': args.cls_lr * 2},
            #{'params': [p for n, p in pooled_output_param_optimizer if any(nd in n for nd in no_decay)],
             #'weight_decay': 0.0,
            #'lr': args.cls_lr * 2},
